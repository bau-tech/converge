...
envsubst '${VITE_NORMALIZER_URL} ${VITE_SPECKLE_SERVER} ${VITE_SPECKLE_TOKEN} ${VITE_SHARE_LINK_MODE} ${VITE_DEMO_LOGIN_EMAIL} ${VITE_DEMO_LOGIN_PASSWORD} ${VITE_EXTRA_SPECKLE_SERVERS} ${VITE_BCF_URL} ${VITE_BCF_API_KEY} ${VITE_OLLAMA_BASE_URL} ${VITE_OLLAMA_MODEL} ${VITE_LMSTUDIO_BASE_URL} ${VITE_LMSTUDIO_MODEL} ${VITE_MISTRAL_API_KEY} ${VITE_ANTHROPIC_API_KEY} ${VITE_COLLABORA_ENABLED}' \
