...
converge_mcp.py :8003  (converge-mcp server)
  FastMCP — 85 tools + 2 resources · ifcopenshell in-memory IFC session
  │
...
FastAPI backend (`:8002`) that ingests Speckle commits into a normalised PostgreSQL schema and serves ~130 REST routes across ingest/sync, models, elements, analytics, filters, 4D timeline/schedule, IFC export (the mature IFC4X3/STEP export, plus an experimental IFC5/`.ifcx` export — buildingSMART's unratified next-gen JSON-based format), IDS checking (including a server-side buildingSMART Data Dictionary proxy powering the IDS editor's property/classification pickers — bSDD blocks real cross-origin browser calls), clash detection, an agentic AI chat assistant (32 BIM/CDE tools, OpenAI/Anthropic/Mistral/local-model providers), report generation (19 report types — bill of materials, data quality, clashes, IDS compliance, room/element schedules, 4D schedule, model changes, BCF coordination, anomalies, and a branded model-summary fact sheet with charts and an optional client-captured 3D view — as real .docx/.xlsx/.pdf files, from one shared `reports/generate.py` gatherer reused by the REST endpoint, the chat assistant, and the MCP server), dashboard auth, Nextcloud-backed documents (ISO 19650 reviewed → approved → verified gate, purpose-of-issue suitability codes, advisory filename-convention checking, and org-scoped WIP visibility so one contractor's work-in-progress stays invisible to another's), a document-workflow notification feed (in-app, plus email if `SMTP_HOST` is configured), dashboard layout/sharing, and debug utilities.
...
A [Model Context Protocol](https://modelcontextprotocol.io/) server — 85 tools + 2 resources — that lets Claude read and reason over your Speckle models: an in-memory `ifcopenshell` IFC session, Speckle GraphQL, normalizer REST queries, filters/overrides, QA/diff/semantic-search intelligence tools, clash/schedule checks, report generation, Nextcloud documents + BCF, and 5D quantity/BoQ export.
...
Major pieces: the 3D viewer (`SpeckleViewer`) and adaptive charts/metrics/tables synced to it by `speckle_id`; a multi-hop element connectivity graph (`ElementConnectivityPanel`) distinguishing real relationship data from geometric-inference edges, with hop-distance opacity and directional arrows; clash (`ClashCheckPanel`) and IDS (`IdsCheckPanel`/`IdsGraphEditor`) checking with viewer highlight and optional BCF topic creation; BCF issue collaboration (`BcfTopicPanel`, `BcfKanbanBoard`); Nextcloud-backed document management with ISO 19650 suitability codes, org-scoped WIP visibility, report generation, and shift/ctrl-click multi-select for bulk move (auto-gating review/approve/verify where the user has the role) or bulk delete (`DocumentsPanel`, multi-format `DocumentPreview`); a document-workflow notification bell covering uploads and every status transition (`NotificationBell`); a native 4D planner (`SchedulePanel` — Gantt + build-up playback); federated multi-model views and cross-model clash checks; an AI chat copilot (`ChatWidget` — OpenAI/Anthropic/Mistral/Ollama/LM Studio); dashboard login (`LoginScreen`/`AuthContext`); and a drag-and-drop, per-project widget layout (`DashboardGrid`).
