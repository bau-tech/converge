...
Set the result as `MCP_API_KEY` in `bim-normalizer/.env`, then rebuild:
```bash
...
```
These are required for SSE (Server-Sent Events) to stream correctly.
...
curl -i -H "X-Api-Key: <your-key>" http://<docker-host-ip>:8003/sse

# Via NPM (public HTTPS)
curl -i -H "X-Api-Key: <your-key>" https://mcp.speckle.example.com/sse
```

Both should return `HTTP/1.1 200` with `Content-Type: text/event-stream`.
A `401 Unauthorized` means the key is wrong or not set in `.env`.
