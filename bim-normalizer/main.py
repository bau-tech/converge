...
def get_parameter_completeness(
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Total element count (denominator)
            ...
            # Per-key fill rate — group by canonical_key first, then raw key
            cur.execute(f"""
                SELECT
                    ...
                    COUNT(DISTINCT CASE WHEN p.value IS NULL OR p.value = '' THEN p.element_id END) AS empty_vals
        ...
        # Aggregate: if canonical_key groups multiple raw keys, merge counts
        from collections import defaultdict
        grouped: dict[str, dict] = {}
        for display_key, canon, raw_key, pset, filled, _empty in rows:
...
# ---------------------------------------------------------------------------
# 4D Schedule (ifc4d / IFC work schedule / P6 XML)
# ---------------------------------------------------------------------------
...
async def import_schedule(model_id: str, file: UploadFile):
    """
    Import a schedule file into bim_tasks for the given model.
    Accepts: .ifc (with IfcWorkSchedule), .xml (Primavera P6 XML).
    """
    from db.connection import get_conn, release_conn
    from db.schedule import import_from_ifc, import_from_p6xml
    ...
    try:
        if filename.endswith('.ifc'):
            ...
            with tempfile.NamedTemporaryFile(suffix='.ifc', delete=False) as f:
                ...
                tmp = f.name
            try:
                result = import_from_ifc(conn, model_id, tmp)
            finally:
                os.unlink(tmp)
        elif filename.endswith('.xml'):
            result = import_from_p6xml(conn, model_id, content)
        else:
            raise HTTPException(status_code=400, detail='Unsupported file type. Upload .ifc or .xml')
        return result
